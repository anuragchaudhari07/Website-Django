...
class DivorceListView(LoginRequiredMixin,ListView):
	model = DivorceComplaint
	def get_queryset(self):
	    return DivorceComplaint.objects.filter(user=self.request.user)

class DivorceDetailView(LoginRequiredMixin,DetailView):
	model = DivorceComplaint

class CreateDivorceView(LoginRequiredMixin,CreateView):
	...
	def get_success_url(self):
		...
		self.object.save()
		return reverse("divorce:detail_d",kwargs={'pk':self.object.dc_id})


class UpdateDivorceView(LoginRequiredMixin,UpdateView):
...
def payment(request, pk):
	if request.method=="POST":
		c_obj = DivorceComplaint.objects.get(pk=pk)
		c_obj.c_amt = request.POST.get('price', '')
		c_obj.save()

		temp = str(c_obj.dc_id)
		o_id = str(uuid.uuid4())
		if c_obj.p_status == 1:
		...
		param_dict={
			...
			'WEBSITE': 'WEBSTAGING',
			'CHANNEL_ID': 'WEB',
			'CALLBACK_URL':'http://127.0.0.1:8000/divorce/' + temp + '/' + 'handlerequest/',
		}
